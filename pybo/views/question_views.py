...
def _list():
    ...
    if kw:
        ...
        question_list = question_list.join(User) \
            ...
            .distinct()
    question_list = question_list.paginate(page=page, per_page=10)   # 한 페이지에 보여할 게시글 수 지정
...
def create():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if image_file:
            ...
            upload_folder = os.path.join(current_app.root_path, 'static/img', today)    # 폴더 경로 지정
        ...
        question = Question(subject=form.subject.data, content=form.content.data, create_date=datetime.now(), user_id=g.user.id, image_path=image_path)
